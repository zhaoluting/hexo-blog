...
### commit
参考[commit辅助工具](https://zhaoluting.github.io/tool-commit/)，项目不强制使用Commitizen等工具，但提交时需要选择性加上以下一种前缀：
- **feat**: 新功能（feature）
- **fix**: 修补bug
- **docs**: 文档改动
- **style**: 不影响代码运行的格式改动
...
- **单文件组件的文件名应始终是单词大写开头 (PascalCase)或横线连接 (kebab-case)。**单词大写开头对于代码编辑器的自动补全最为友好，因为这使得我们在 JS(X) 和模板中引用组件的方式尽可能的一致。（涉及到[__大小写不敏感的文件系统的问题__](https://zhaoluting.github.io/tool-caseSensitive/)，待讨论）
